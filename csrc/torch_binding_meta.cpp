    ...
    c10::string_view quant_mode)
{
    ...
    return {
        at::empty_symint(y_shape, x.options().dtype(at::kChar)),
        at::empty_symint(scale_shape, x.options().dtype(at::kFloat)),
    };
}
    ...
    int64_t dst_type)
{
    ...
    TORCH_CHECK(dst_type == DST_TYPE_E4M3FN || dst_type == DST_TYPE_E5M2,
                "situ_mx_quant: unsupported dst_type ", dst_type);
    (void)linear_beta;
    (void)activate_left;
    c10::SymDimVector y_shape(x.sym_sizes().begin(), x.sym_sizes().end());
    y_shape.back() = y_shape.back() / 2;
    c10::SymDimVector scale_shape(y_shape.begin(), y_shape.end());
    scale_shape.back() = (scale_shape.back() + MX_BLOCK_SPAN - 1) / MX_BLOCK_SPAN;
    scale_shape.emplace_back(MX_SCALE_ALIGN);
    auto y_dtype = dst_type == DST_TYPE_E5M2 ? at::kFloat8_e5m2 : at::kFloat8_e4m3fn;
    return {
        at::empty_symint(y_shape, x.options().dtype(y_dtype)),
