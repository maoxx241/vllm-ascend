...
def _runner_methods():
    ...
    selected = {
        "_align_memory", "_allocate_int8_cache_tensor", "_get_layer_kv_cache_specs",
        "_allocate_kv_cache_tensors", "_reshape_kv_cache_tensors",
    }
...
def test_va_shared_backing_preserves_mixed_views_and_page_holes(with_kv_transfer):
    ...
    def attention_spec(heads, head_dim):
        return SimpleNamespace(
            block_size=block_size, num_heads=heads, num_states=block_size,
            state_content_size_bytes=head_dim * 2, dtype=torch.bfloat16,
    ...
    mamba_spec = _MambaSpec(
        shapes=shapes, dtypes=dtypes, num_heads=1, num_states=1,
    ...
    config = SimpleNamespace(
        ...
        kv_cache_tensors=[
            SimpleNamespace(size=size, layers=group.layer_names, offset=offset + slot * layer_stride,
                            layer_stride=layer_stride, block_stride=pitch)
