...
class _FakeBackendMeta(type):
    def __ror__(cls, other):
        return object()


class FakeBackend(str, metaclass=_FakeBackendMeta):
    pass
...
def _load_patch_distributed_module():
    ...
    torch_distributed_c10d = ModuleType("torch.distributed.distributed_c10d")
    ...
    shm_broadcast_module = ModuleType(
    ...
    vllm_ascend_module = ModuleType("vllm_ascend")
    vllm_ascend_patch = ModuleType("vllm_ascend.patch")
    vllm_ascend_patch_worker = ModuleType("vllm_ascend.patch.worker")
    vllm_ascend_distributed = ModuleType("vllm_ascend.distributed")
    vllm_ascend_device_communicators = ModuleType(
        "vllm_ascend.distributed.device_communicators"
    )
    npu_communicator_module = ModuleType(
