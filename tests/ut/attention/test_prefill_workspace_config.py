...
def test_workspace_default_and_override():
    assert AscendConfig().chunked_prefill_workspace_max_tokens == 128 * 1024
    assert (
        AscendConfig(chunked_prefill_workspace_max_tokens=512 * 1024).chunked_prefill_workspace_max_tokens == 512 * 1024
    )
...
def test_workspace_rejects_invalid_config(value):
    with pytest.raises(ValidationError, match="chunked_prefill_workspace_max_tokens"):
        AscendConfig(chunked_prefill_workspace_max_tokens=value)
...
def test_workspace_sizing_preserves_heuristic(cap, max_model_len, max_num_seqs, block_size, expected):
    config = AscendConfig(chunked_prefill_workspace_max_tokens=cap)
