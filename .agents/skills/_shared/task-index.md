...
| Performance analysis | first-hop classify perf request -> if evidence is thin, ask for minimum artifact bundle -> otherwise route to perf-hunter and add graph-analysis or matrix planning when needed | [imported knowledge report](knowledge-governance/generated/imported_knowledge_report.json) -> [code-knowledge-map](code-knowledge-map.md) | `E1 vllm-ascend-developer-assistant -> C6 vllm-ascend-perf-assistant` |
