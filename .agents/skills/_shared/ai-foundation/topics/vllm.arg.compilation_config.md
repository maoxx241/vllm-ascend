...
- constraints: --cudagraph-capture-sizes 与 compilation_config.cudagraph_capture_sizes 互斥。; --max-cudagraph-capture-size 与 compilation_config.max_cudagraph_capture_size 互斥。; 若 cudagraph_mode 需要 piecewise，但 mode 非 VLLM_COMPILE，会被覆盖到 NONE。
- combo_effects: 与 --enforce-eager 冲突：eager 打开会清空 cudagraph 相关设置。; 与 --optimization-level 叠加决定最终编译策略。
