...
- constraints: xlite_graph_config.enabled=true 时要求 block_size=128 且不兼容 speculative decoding; weight_prefetch_config 需结合并发与模型类型调优 prefetch_ratio
- combo_effects: 与 --compilation-config、--block-size、并行参数共同决定最终执行路径; 与部分环境变量存在兼容层（如 VLLM_ASCEND_ENABLE_PREFETCH_MLP）
