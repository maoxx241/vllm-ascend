...
class AscendAttentionBackend(AttentionBackend):
    ...
    def customize_spec(cls, spec: AttentionSpec) -> AttentionSpec:
        ...
        if spec.dtype not in (torch.bfloat16, torch.float16) or spec.kv_quant_mode.name != "NONE" or spec.tokens_per_state != 1:
...
def _build_flash_attention_metadata(builder, common, *, is_mla: bool) -> AscendFlashAttentionMetadata:
    ...
    if key not in builder._flash_buffers:
        ...
        shape = (tokens, builder.flash_num_heads, head_dim)
        kv_heads = 1 if is_mla else builder.flash_num_kv_heads
        words = ((36 + 72) * rows * kv_heads + 1) * 16
    ...
    def build_metadata() -> None:
        ...
        if is_mla:
            ...
            metadata = flash_attn_metadata(
                builder.flash_num_heads, builder.flash_num_kv_heads, 64,
                cu_seqlens_q=flash.cu, seqused_q=flash.used_q,
                seqused_kv=flash.cache_lens, batch_size=batch + 1,
                max_seqlen_q=flash.max_query_len, max_seqlen_kv=flash.max_seq_len,
    ...
    def build(
        ...
        if envs.VLLM_ASCEND_ENABLE_FLASH_MLA:
            ...
            return self.metadata_cls(
                ...
                seq_lens=common.seq_lens, query_start_loc=common.query_start_loc,
                block_tables=common.block_table_tensor, slot_mapping=common.slot_mapping,
                max_query_len=common.max_query_len, causal=common.causal,
...
class AscendAttentionBackendImpl(AttentionImpl):
    def __init__(
        ...
        **kwargs,
        ...
        if envs.VLLM_ASCEND_ENABLE_FLASH_MLA and (
            head_size != 64 or alibi_slopes is not None or sliding_window is not None
            or logits_soft_cap not in (None, 0) or sinks is not None
            or attn_type != AttentionType.DECODER or kv_sharing_target_layer_name is not None
        ):
    ...
    def _forward_flash(self, layer, query, key, value, kv_cache, attn_metadata, output):
        ...
        if key is not None and value is not None:
            torch_npu.npu_scatter_pa_kv_cache(
                key=key[:tokens].contiguous(), value=value[:tokens].contiguous(),
                key_cache=key_cache.permute(0, 2, 1, 3),
                value_cache=value_cache.permute(0, 2, 1, 3),
                slot_mapping=flash.slots, cache_mode="Norm",
            )
        notify_kv_cache_written()
        result, _ = flash_attn(
            flash.query, key_cache, value_cache,
            block_table=flash.block_table, cu_seqlens_q=flash.cu,
            seqused_q=flash.used_q, seqused_kv=flash.cache_lens,
            attn_mask=flash.attn_mask if attn_metadata.causal else None,
            metadata=flash.schedule, softmax_scale=self.scale,
