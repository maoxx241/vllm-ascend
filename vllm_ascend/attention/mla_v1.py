...
class AscendMLAImpl(MLAAttentionImpl):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if envs.VLLM_ASCEND_ENABLE_FLASH_MLA:
            ...
            self.enable_mlapo = not self.is_draft_model and not enable_dcp() and not self.use_mla_rope
    ...
    def mla_preprocess_only_decode(self, hidden_states, kv_cache, attn_metadata):
        if envs.VLLM_ASCEND_ENABLE_FLASH_MLA:
            ...
            cache_index = attn_metadata.flash.slots
    ...
    def _forward_flash(self, layer_name, hidden_states, kv_cache, attn_metadata, output=None):
        ...
        if t == 0:
            return output.zero_()
        x = hidden_states[:t]
        use_mlapo = self.enable_mlapo and not b.is_prefill and b.dcp_size == 1 and t <= MLAPO_MAX_SUPPORTED_TOKENS
        if use_mlapo:
