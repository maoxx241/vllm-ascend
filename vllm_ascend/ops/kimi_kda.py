    ...
    *,
    ...
    if loaded_weight.ndim == 1:
        if loaded_weight.shape[0] < num_heads:
            raise ValueError(
                f"Kimi K3 A_log has fewer checkpoint heads than the model: {loaded_weight.shape[0]} < {num_heads}"
            )
        # Official K3 stores 96 logical heads followed by 32 zero-padding
        # elements in a one-dimensional [128] tensor.
        ...
        if loaded_weight.shape[0] != 1 or loaded_weight.shape[1] != 1 or loaded_weight.shape[3] != 1:
            raise ValueError(
                f"Kimi K3 A_log 4-D checkpoint must have shape [1, 1, H, 1], got {tuple(loaded_weight.shape)}"
            )
        ...
        if loaded_weight.shape[2] < num_heads:
            raise ValueError(
                f"Kimi K3 A_log has fewer checkpoint heads than the model: {loaded_weight.shape[2]} < {num_heads}"
            )
        loaded_weight = loaded_weight[:, :, :num_heads, :]
    else:
        raise ValueError(f"Kimi K3 A_log checkpoint must be 1-D or 4-D, got {loaded_weight.ndim}-D")

    local_heads = param.shape[2]
    if local_heads <= 0 or num_heads % local_heads != 0:
        raise ValueError(
            "Kimi K3 A_log parameter shape is incompatible with logical heads: "
            f"param={tuple(param.shape)}, num_heads={num_heads}"
        )
    tp_rank = get_tensor_model_parallel_rank()
    start = tp_rank * local_heads
    if start + local_heads > num_heads:
        raise ValueError(f"Kimi K3 A_log TP rank {tp_rank} exceeds {num_heads} logical heads")
...
class AscendKimiGatedDeltaNetAttention(KimiGatedDeltaNetAttention):
    ...
    def __init__(self, config, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        # This must live on the OOT Ascend layer: constructing by the upstream
        # class name replaces that class before model-specific subclasses can
        # run. Keep it exact to K3 so Kimi K2's checkpoint contract is unchanged.
        if isinstance(config, KimiK3TextConfig):
            self.A_log.weight_loader = partial(
                _load_kimi_k3_a_log,
                num_heads=self.num_heads,
            )

        # Kimi K3 checkpoints may expose the low-rank output gate even when the
        # current execution path does not use it.
