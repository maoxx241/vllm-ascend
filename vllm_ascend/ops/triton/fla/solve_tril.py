...
def solve_tril(
    ...
    chunk_indices = (
        launch_plan.get_block_indices(LARGE_BLOCK_T)
        if launch_plan is not None and cu_seqlens is not None
        else prepare_chunk_indices(cu_seqlens, LARGE_BLOCK_T)
        if cu_seqlens is not None
        else None
    )
    NT = (
        launch_plan.get_num_blocks(LARGE_BLOCK_T)
        if launch_plan is not None and cu_seqlens is not None
    ...
    chunk_indices = (
        launch_plan.get_block_indices(BT)
        if launch_plan is not None and cu_seqlens is not None
