...
def _w4a16_mxfp_situ_apply_mlp(
    *,
    ...
    """Apply W4A16 MXFP as GMM1 -> SiTU -> GMM2.

    W4A16 keeps activations in FP16/BF16, so it must not enter the W4A8
    dynamic-MX activation quantization path.
    """
    w1_tensor = _require_single_tensor_for_swiglu_quant(w1, name="w1")
    w1_scale_tensor = _require_single_tensor_for_swiglu_quant(
        w1_scale,
        name="w1_scale",
    )
    cumulative_group_list = cumsum_group_list(
        group_list,
        group_list_type,
        0,
    )
    gate_up_out = torch_npu.npu_grouped_matmul(
        x=[hidden_states],
        weight=[w1_tensor],
        antiquant_scale=[w1_scale_tensor],
    ...
    hidden_states = DeviceOperator.npu_grouped_matmul_gmm2(
        ...
        use_mxfp_quant=True,
        bias=None,
        fallback_output_dtype=hidden_states.dtype,
...
def _w4a8_situ_apply_mlp(
    *,
    ...
    if use_mxfp_quant:
        hidden_states, situ_out_scale = mxfp_situ_quant(gate_up_out, activation)
    else:
        hidden_states, situ_out_scale = dynamic_situ_quant(gate_up_out, activation)
    before_gmm2_evt = torch.npu.current_stream().record_event()
    hidden_states = DeviceOperator.npu_grouped_matmul_gmm2(
        hidden_states=hidden_states,
        weight=w2,
...
def quant_apply_mlp(
    ...
    if situ_activation is not None and mxfp_quant_dtype is QuantType.W4A16MXFP:
        return _w4a16_mxfp_situ_apply_mlp(
            hidden_states=hidden_states,
            w1=w1,
            w1_scale=w1_scale,
            w2=w2,
