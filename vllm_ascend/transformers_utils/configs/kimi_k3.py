...
class KimiK3TextConfig(KimiLinearConfig):
    ...
    modelslim_packed_modules_mapping_key = "kimi_k3"
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if self.linear_attn_config is not None:
            self.linear_num_value_heads = self.linear_attn_config["num_heads"]
...
def register_kimi_k3_config() -> None:
    """Register K3 with Transformers before vLLM resolves the checkpoint."""

    AutoConfig.register(
        KimiK3Config.model_type,
        KimiK3Config,
        exist_ok=True,
    )
