...
class AscendW4A16MXFP4FusedMoEMethod(AscendMoEScheme):
    ...
    def process_weights_after_loading(self, layer):
        w13_weight = getattr(layer, "w13_weight_packed" if self.use_weight_packed else "w13_weight")
        w2_weight = getattr(layer, "w2_weight_packed" if self.use_weight_packed else "w2_weight")
