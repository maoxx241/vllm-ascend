...
def _uses_projector_rotation(quant_config: QuantizationConfig | None) -> bool:
    """Whether this checkpoint declares the A3 ModelSlim projector transform."""
    ...
    quant_type = quant_config.quant_description.get("mm_projector.rot_proj.weight")
    if quant_type is None:
        return False
    if quant_type != "FLOAT":
        raise ValueError(
            f"Kimi K3 mm_projector.rot_proj must be FLOAT in the ModelSlim quantization description, got {quant_type!r}"
        )
    return True
...
class AscendKimiK3ForConditionalGeneration(
    ...
    packed_modules_mapping = AscendKimiK3ForCausalLM.packed_modules_mapping
    ...
    def __init__(self, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        with self._mark_tower_model(vllm_config, "vision_chunk"):
            ...
            self.mm_projector = KimiK3MultiModalProjector(
                ...
                enable_rotation=_uses_projector_rotation(self.quant_config),
            )
    ...
    def _parse_and_validate_media_input(self, **kwargs: object) -> KimiK3MediaPixelInputs | None:
        ...
        if pixel_values.ndim == 5:
            ...
            raise ValueError(
                "Kimi K3 pixel_values must have shape [patches, 3, height, width] "
                f"or a batched five-dimensional equivalent, got {tuple(pixel_values.shape)}"
            )
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        rotation_name = "mm_projector.rot_proj.weight"
        if self.mm_projector.rot_proj is not None and rotation_name not in loaded:
            raise ValueError(
                "ModelSlim metadata enables Kimi K3 projector rotation, but "
                f"the checkpoint does not contain {rotation_name}"
            )
        return loaded
