...
from vllm_ascend.ops.mla import AscendMLAFeatures, AscendMLAModules
...
def _situ_params(config: KimiK3TextConfig) -> tuple[float, float | None]:
    beta = 1.0 if config.activation_situ_beta is None else config.activation_situ_beta
    return beta, config.activation_situ_linear_beta


def _uses_global_inputs_embeds(vllm_config: VllmConfig, modality: str) -> bool:
    """Whether layer zero receives embeddings before FlashComm sharding."""

    model_config = vllm_config.model_config
    if model_config.enable_prompt_embeds:
        return True
    multimodal_config = model_config.multimodal_config
    return multimodal_config is not None and (
        multimodal_config.enable_mm_embeds or multimodal_config.get_limit_per_prompt(modality) > 0
    )
...
class _KimiK3TPMergedColumnParallelLinear(AscendMergedColumnParallelLinear):
...
class _KimiK3TPRowParallelLinear(AscendRowParallelLinear):
    ascend_parallel_mode = AscendLinearParallelMode.TENSOR_PARALLEL


class KimiK3MLP(nn.Module):
    gate_up_proj_cls = MergedColumnParallelLinear
    down_proj_cls = RowParallelLinear
    ...
    def __init__(
        ...
        self.gate_up_proj = self.gate_up_proj_cls(
            hidden_size,
        ...
        self.down_proj = self.down_proj_cls(
            intermediate_size,
            hidden_size,
            bias=False,
            quant_config=quant_config,
...
class KimiK3SharedExpertMLP(KimiK3MLP):
    """K3 shared experts remain TP-sharded under FlashComm."""

    gate_up_proj_cls = _KimiK3TPMergedColumnParallelLinear
    down_proj_cls = _KimiK3TPRowParallelLinear
...
class KimiK3MoE(nn.Module):
    def __init__(
        ...
        if config.num_shared_experts is None or config.num_shared_experts <= 0:
            raise ValueError("Kimi K3 requires at least one shared expert")
        ...
        self.shared_experts = KimiK3SharedExpertMLP(
            config,
        ...
        self.experts = FusedMoE(
            ...
            # SiTU has the same gated expert-weight layout as SiLU.  Keep the
            # upstream factory contract standard and pass the Ascend execution
            # semantics through its documented runner extension point.
            activation="silu",
            runner_cls=AscendMoERunner,
            runner_args={
                "ascend_activation": SituActivationConfig(
                    beta=beta,
                    linear_beta=linear_beta,
                ),
                "n_shared_experts": self.num_shared_experts,
                "shared_experts_use_tp": True,
            },
        )
...
class KimiK3MLAAttention(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        gate_projection_cls = (
            _KimiK3TPColumnParallelLinear if input_layout is AscendTokenLayout.GLOBAL else ColumnParallelLinear
        )
        self.g_proj = gate_projection_cls(
            hidden_size,
...
class KimiK3DecoderLayer(nn.Module):
    def __init__(self, config: KimiK3TextConfig, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        self.input_layout = (
            ...
            if self.layer_idx == 0
            and _uses_global_inputs_embeds(
                vllm_config,
                "vision_chunk",
            )
    ...
    def forward(
        ...
        if self.input_layout is AscendTokenLayout.GLOBAL and _EXTRA_CTX.flash_comm_v1_enabled:
            tp_size = get_tensor_model_parallel_world_size()
